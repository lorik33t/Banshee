...
console.error('[Bridge] Starting Claude bridge with PTY handler (CJS)...');

// Use the PTY handler instead of spawning Claude directly (CJS variant)
const handlerPath = path.join(__dirname, 'claude-handler.cjs');
...
// Forward handler output to stdout (line-buffered so Rust line reader emits promptly)
let __buffer = '';
handler.stdout.on('data', (data) => {
  __buffer += data.toString();
  let idx;
  while ((idx = __buffer.indexOf('\n')) !== -1) {
    const line = __buffer.slice(0, idx);
    __buffer = __buffer.slice(idx + 1);
...
console.error('[Bridge] Ready with PTY handler (CJS)');
